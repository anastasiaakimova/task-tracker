...
import by.akimova.task.tracker.api.factories.TaskStateDtoFactory;
import by.akimova.task.tracker.store.entities.ProjectEntity;
import by.akimova.task.tracker.store.entities.TaskStateEntity;
import by.akimova.task.tracker.store.repositories.TaskStateRepository;
...
public class TaskStateController {
    ...
    public List<TaskStateDto> getTaskStates(
        ...
        var project = controllerHelper.getProjectOrThrowException(projectId);
    ...
    public TaskStateDto createTaskState(
        ...
        var taskState = taskStateRepository.saveAndFlush(TaskStateEntity.builder()
                .name(taskStateName)
                .project(project)
                .build()
        );
    ...
    public TaskStateDto updateTaskState(
        ...
        var taskState = getTaskStateOrThrowException(taskStateId);
        ...
        taskStateRepository
                ...
                .ifPresent(anotherTaskState -> {
                    throw new BadRequestException(
                            String.format(
                                    "Task state \"%s\" already exists", taskStateName));
                });
